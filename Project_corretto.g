...
definizione_stato
	...
	{ sem.registraStato(nomeStato); }
			OPG 
				attributo ( COMMA attributo )* 
			CPG 
	;
attributo [Token nomeStato]
	:	nomeAttributo=ID 
		OPT 
			nomeOggetto=OGGETTO  {sem.registraAttributo(nomeStato, nomeAttributo, nomeOggetto);}
		CPT 
		;
...
definizione_operatore
	:	OPERATORE {x=sem.registraOperatore();} 
			OPG 
				a=azione COMMA 					{x.addAzione(a);}
				p=precondizioni COMMA 	{x.addPrecond(p);}
				e=effetti COMMA 				{x.addEffect(e);}
				c=costo 								{x.addCost(c);}
			CPG
		;

precondizioni
	:	p=PRECONDIZIONI EQ OPG la=lista_attributi_variabili[$p]  CPG;
...
effetti	
	:	effects=EFFETTI EQ OPG la=lista_attributi_variabili[effects] CPG;
